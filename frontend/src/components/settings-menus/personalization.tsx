...
export default function PersonalizationSettingsMenu() {
  const [firmwareSettings, setFirmwareSettings] =
    useState<WebMelonFirmwareSettings>(
  ...
  return (
    <div className="personalization-container">
      ...
      <input
        ...
        placeholder="Name"
        maxlength={10}
        value={firmwareSettings.nickname}
        onChange={setNickname}
        className="input input-bordered w-full"
      />
      <div className="label">
        <span class="label-text-alt">
          This name will be shown in games and menus inside the emulator. Max
        ...
        <div className="birthday-selector">
          ...
          >
            {Array.from({ length: 12 }, (_, i) => i + 1).map((i) => (
              <option key={i} selected={i === firmwareSettings.birthdayMonth}>
                {i}
              </option>
            ))}
          </select>
            ...
            {Array.from(
              {
                length: new Date(
                  2008,
                  firmwareSettings.birthdayMonth,
                  0,
                ).getDate(),
              },
              (_, i) => i + 1,
            ).map((i) => (
              <option key={i} selected={i === firmwareSettings.birthdayDay}>
                {i}
              </option>
            ))}
          </select>
      ...
      <div className="label">
        <span class="label-text-alt">
          Some games may add easter eggs on this day. This doesn't have to be
          your real birthday.
        </span>
      </div>
      <div className="label">
      ...
      <div className="birthday-selection">
        <div className="birthday-selector">
          ...
          >
            {window.WebMelon.constants.FIRMWARE_LANGUAGES.map(
              (language: any) => (
                <option
                  key={language.id}
      ...
      <div className="label">
        <span class="label-text-alt">
          This language will be used as the firmware language and may be used by
          the games in the emulator as well.
        </span>
      </div>
    </div>
  );
