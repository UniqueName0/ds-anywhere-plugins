...
template <CPUExecuteMode cpuMode>
u32 NDS::RunGhostFrame()
{
    // The intention of this function is to run a "ghost frame" which runs the CPU for a frame without
    // actually rendering the 3D aspects on a GPU. It does not currently work, and it has been abandoned
    // since I am not finding a need for it anymore, but I'm keeping the function around just in case
    // I do need it later.

    // Currently, when a frame is dropped, the emulator clock lags behind, meaning that time will appear
    // 2x slower to the user which is not ideal, especially in multiplayer sessions. The main reason
    // frame rates decrease is because 3D rendering is taking too long, so skipping the 3D render in
    // some frames would be a great solution to running at a lower real frame rate on lower-end devices.
...
#endif
    {
        return RunGhostFrame<CPUExecuteMode::Interpreter>();
    }
}
